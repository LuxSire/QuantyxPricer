"""Interest Rate Floor pricer — sum of Black floorlets.

Each floorlet pays max(K − F_i, 0) on the period notional at the payment date.

Black floorlet formula (log-normal):
  d1 = [ln(F/K) + ½σ²T] / (σ√T)
  d2 = d1 − σ√T
  floorlet_PV = notional × α_i × DF(T_i) × [K × N(−d2) − F_i × N(−d1)]

Normal (Bachelier) floorlet formula:
  floorlet_PV = notional × α_i × DF(T_i) × [(K−F) × N(−z) + σ√T × n(z)]
  where z = (F − K) / (σ√T),  n() = standard normal PDF

Put-call parity with cap:  floor_PV = cap_PV − swap_PV
(not enforced here; each is priced independently from the same vol)

Required JSON fields
--------------------
  instrument_id      ISIN or internal identifier
  evaluation_date    Pricing date (DD-MM-YYYY or YYYY-MM-DD)
  start_date         Effective start of the floor
  maturity_date      Final floorlet payment date
  notional           Notional amount
  strike_rate        Floor strike rate (decimal or %)
...
ASSET_FILE = ASSETS_DIR / 'floor_example.json'

_N  = ql.CumulativeNormalDistribution()
_SN = ql.NormalDistribution()


def _black_floorlet(fwd, strike, vol, T, alpha, df, notional):
    """Black log-normal floorlet PV."""
    if T <= 0 or vol <= 0 or fwd <= 0 or strike <= 0:
        return max(strike - fwd, 0.0) * alpha * df * notional
    sqrt_T = math.sqrt(T)
    d1 = (math.log(fwd / strike) + 0.5 * vol * vol * T) / (vol * sqrt_T)
    d2 = d1 - vol * sqrt_T
    return notional * alpha * df * (strike * _N(-d2) - fwd * _N(-d1))


def _normal_floorlet(fwd, strike, vol, T, alpha, df, notional):
    """Bachelier normal floorlet PV."""
    if T <= 0 or vol <= 0:
        return max(strike - fwd, 0.0) * alpha * df * notional
    sqrt_T = math.sqrt(T)
    z = (fwd - strike) / (vol * sqrt_T)
    return notional * alpha * df * ((strike - fwd) * _N(-z) + vol * sqrt_T * _SN(z))
...
def price_asset(data, curve_json):
    ...
    total_pv  = 0.0
    floorlets = []
    pricer    = _black_floorlet if vol_type == 'black' else _normal_floorlet
    ...
    for i in range(1, len(dates)):
        ...
        floorlets.append({
            ...
            'strike':        strike,
            'df':            df1,
            'intrinsic':     max(strike - fwd, 0.0) * alpha * df1 * notional,
            'pv':            pv,
        })
...
def print_result(data, result):
    ...
    print(f"Strike (floor rate): {result['strike_rate']:.4%}")
    print(f"Flat vol ({result['vol_type']}): {result['flat_vol']:.4%}")
    print(f"Number of floorlets: {result['num_floorlets']}")
    print(f"NPV (floor):       {result['npv']:,.2f}")
    print(f"Curve:             {result['curve_name']}")
    if result['floorlets']:
        print(f"\n{'Period end':<14} {'Fwd':>8} {'Strike':>8} {'T_fix':>6} {'DF':>8} {'PV':>12}")
        for f in result['floorlets']:
            print(f"{f['period_end']:<14} {f['fwd_rate']:>8.4%} {f['strike']:>8.4%} "
                  f"{f['fixing_time']:>6.4f} {f['df']:>8.6f} {f['pv']:>12,.2f}")
    print()


def parse_args():
    parser = argparse.ArgumentParser(description='Price an Interest Rate Floor.')
    parser.add_argument('--bond-file',  default=str(ASSET_FILE),  help='Path to floor JSON')
